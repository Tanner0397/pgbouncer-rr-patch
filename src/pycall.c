...
char *pycall(PgSocket *client, char *username, char *database, char *query_str, char *py_file,
		char* py_function) {
	...
	/* Call function with two arguments - username, database,  query_str */
	pArgs = PyTuple_New(3);
	if (pArgs == NULL) {
	...
	pValue = PyUnicode_FromString(database);
	if (pValue == NULL) {
		slog_error(client, "Python module <%s>: out of memory", py_module);
		goto finish;
	}
	...
	if (PyUnicode_Check(pValue)) {
		res = strdup(PyUnicode_AsUTF8(pValue));
	} else {
		res = NULL;
	}
	...
	if (PyErr_Occurred()) {
		...
		slog_error(client, "Python error: %s", PyUnicode_AsUTF8(perror));
	}
	free(py_pathtmp);
	free(py_filetmp);
	free(py_path);
